...
image.shape
imshow('Bottle in gray', image)
decoded_text, info_string, points, _ = qr_code_detector.detectAndDecodeMulti(
    image)
points_dict = {k: points[i].reshape(-1, 2) for i, k in enumerate(info_string)}
...
centers[0]
for c in centers:
    cv2.drawMarker(image_org, c, (0, 255, 0), 	markerSize=200,	thickness=20)

imshow('Bottle in color with marked QR', image_org)
...
image_cropped_org = image_org[roi_top_left[1]
    :roi_bottom_right[1], roi_top_left[0]:roi_bottom_right[0]]
...
imshow('Roi H S V', roi_component)
# imshow('Space between', roi_hue)
...
imshow('H S V with bar next to, calculated as mean from every row', roi_component_mean)
image_roi_gy.shape
roi_bar.shape
image_roi_gy_crop_maen.shape
...
fillment = len(roi_val_mean_grad[idx:]) / len(roi_val_mean_grad)
...
cv2.putText(image_cropped_org_m, '{:2.1f}%'.format(
    fillment*100), (int(roi_top_left[0]), roi_top_left[1]+idx), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 0), thickness=5)
imshow('Space between', image_cropped_org_m)
